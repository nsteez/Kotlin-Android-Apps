...
class GamePlay : AppCompatActivity()  {
    ...
    val gamePlayToMain = Intent(this,MainActivity::class.java)
    var numberOfGuess = 3;
    ...
    val wordToGuessVal = intent.getStringExtra("Type").toString()
        ...
        if (guess == wordToGuessVal) {
            ...
            val timer = object: CountDownTimer(2000, 1000) {
                override fun onTick(millisUntilFinished: Long) {
                    word.text = wordToGuessVal;
                }
                override fun onFinish() {
                    Log.d("setting wordToGuess text", guess);
                    resetGame()
                    winToast.show()
                    startActivity(gamePlayToMain)
                }
            }
            timer.start()
            return
